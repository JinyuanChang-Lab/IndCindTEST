...
Data1 <- function(n, p, m, dep) {
  ...
  v <- matrix(rnorm(n * q), nrow = n, ncol = q)
  z <- matrix(rnorm(n * m), nrow = n, ncol = m)
  x <- matrix(rnorm(n * p), nrow = n, ncol = p)
  y <- matrix(rnorm(n * q), nrow = n, ncol = q)
  
  # Process x and y according to dep
  if (dep == 0) {
    for (i in 1:(p %/% 4)) {
      zz <- rowMeans(z[, 1:m, drop = FALSE])  # # Mean of the first m columns of z for each row
...
res_my <- function(n, p, k, dep, alpha, rep.num, mycores){ 
  ...
  res_gauss <- mclapply(1:rep.num, function(rep.sim){                     # Use mclapply for parallel computation 
    if(rep.sim %% 200 == 0)  cat('---- Test times = ', rep.sim, '----\r',file = "ex10log.txt", append=TRUE)
  ...
  cat("---- result=\n", file = "ex10log.txt", append = TRUE)
  capture.output(print(result), file = "ex10log.txt", append = TRUE)
  cat("\n----\n", file = "ex10log.txt", append = TRUE)
  return(result)
}
...
for (n in c(100)) {
  for (dep in c(0, p/10, p/5)) {
    ...
    write.csv(
      result_m,
      paste0("cex10", "_n", n, "_p", p, "_a", alpha*100, "_dep", dep, ".csv"),
      row.names = FALSE
    )
