...
for n_idx = 1:length(n_values)
    ...
    for scenario_idx = 1:length(scenarios)
        scenario = scenarios{scenario_idx};  % choose scenario  
        for idx = 1:length(p_values)
            ...
            parfor i=1 : n_my
                ...
                %generate data
                [x, y, Sig1] = gen_gauss_XY(n,p1,p2, scenario);

                % the proposed method
                res1 = Ind_Gtest_mat(x, y, alpha, 'Rademacher', 5000, n*1000+123);
...
for n_idx = 1:length(n_values)
    ...
    for idx = 1:length(p_values)
        ...
        parfor i=1 : n_my
            %tic
            rng(i*1000+ idx);
            [x, y] = generate_gauss_data(n, p1, p2);
            res1 = Ind_Gtest_mat(x, y, alpha, 'Rademacher', 5000, n*1000+123);
            res_my(i,:) = res1.reject;
            X = [x, y];
