...
res_my <- function(n, p, k, dep, alpha, rep.num, mycores){ 
  ...
  res_gauss <- mclapply(1:rep.num, function(rep.sim){                     # Use mclapply for parallel computation 
    if(rep.sim %% 200 == 0)  cat('---- Test times = ', rep.sim, '----\r',file = "ex8log.txt", append=TRUE)
  ...
  cat("---- result=\n", file = "ex8log.txt", append = TRUE)
  capture.output(print(result), file = "ex8log.txt", append = TRUE)
  cat("\n----\n", file = "ex8log.txt", append = TRUE)
  return(result)
}
...
for (n in c(100)) {
  for (dep in c(0, p/10, p/5)) {
    ...
    write.csv(
      result_m,
      paste0("cex8", "_n", n, "_p", p, "_a", alpha*100, "_dep", dep, ".csv"),
      row.names = FALSE
    )
