...
source('./ind_functions_dep/ind-functions.R')
source('./ind_functions_dep/PIND.R')
...
#####generate data in Section R.1
Data1 <- function(n, p, q, rho = 0.8) {
  
  # i.i.d. N(0,1) bases
  Z1 <- matrix(rnorm(n * p), n, p)  # will be X
  Z2 <- matrix(rnorm(n * p), n, p)  # independent of Z1
  
  # Rademacher signs S in {-1, +1}
  S  <- matrix(sample(c(-1, 1), n * p, replace = TRUE), n, p)
  
  # Construct pairs with correlation = rho * S_ij
  X <- Z1
  Y <- rho * S * Z1 + sqrt(1 - rho^2) * Z2
  
  # E[XY]=0 but X and Y are not independent.
  list(x = X, y = Y, S = S)
}
...
res_my <- function(n, p, q, dep, alpha, rep.num, mycores){   
  ...
  res_gauss <- mclapply(1:rep.num, function(rep.sim){      # Use mclapply for parallel computation             
    ...
    if(rep.sim %% 200 == 0)  cat('---- Test times = ', rep.sim, '----\r',file = "nopowerlog.txt", append=TRUE)
  ...
  result <- data.frame(list(method=c("gaussian","mammer", "radmacher", "Pcor", "rdCov", "dCor", "dHSIC", "JdCovR", "GdCov", "Hallin", "mrdCov"),
  ...
  cat("---- result=\n", file = "nopowerlog.txt", append = TRUE)
  capture.output(print(result), file = "nopowerlog.txt", append = TRUE)
  cat("\n----\n", file = "nopowerlog.txt", append = TRUE)
  return(result)
}
...
for (n in nall) {
  ...
  for (dep in c(0.8)) {
    ...
    cat("...........n_", n, "...........\n")
    cat("...........dep_", dep, "...........\n")
    # cat('---- n = ', n, '----\r',file = "ex6log.txt", append=TRUE)
    # cat('---- p = ', p, '----\r',file = "ex6log.txt", append=TRUE)
    # cat('---- dep = ', dep, '----\r',file = "ex6log.txt", append=TRUE)
    ...
    write.csv(
      result_m,
      paste0("exnopower", "_n", n, "_p", p, "_a", alpha*100, "_dep", dep, ".csv"),
      row.names = FALSE
    )
