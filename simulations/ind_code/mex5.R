...
Data1 <- function(n, p, q, d){
  ...
  if(d==0){
    ...
    x <- matrix(sapply(xx, funmy),n,p)
    y <- matrix(sapply(yy, funmy),n,p)
  }
  dat <- list(x=x, y=y)
  return(dat)  
}
...
res_my <- function(n, p, q, dep, alpha, rep.num, mycores){   
  ...
  res_gauss <- mclapply(1:rep.num, function(rep.sim){      # Use mclapply for parallel computation             
    ...
    if(rep.sim %% 200 == 0)  cat('---- Test times = ', rep.sim, '----\r',file = "ex5log.txt", append=TRUE)
  ...
  cat("---- result=\n", file = "ex5log.txt", append = TRUE)
  capture.output(print(result), file = "ex5log.txt", append = TRUE)
  cat("\n----\n", file = "ex5log.txt", append = TRUE)
  return(result)
}
...
for (n in nall) {
  ...
  for (dep in c(0, 1)) {
    ...
    write.csv(
      result_m,
      paste0("ex5", "_n", n, "_p", p, "_a", alpha*100, "_dep", dep, ".csv"),
      row.names = FALSE
    )
