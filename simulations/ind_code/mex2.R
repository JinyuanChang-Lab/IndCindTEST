...
res_my <- function(n, p, q, dep, alpha, rep.num, mycores){   
  ...
  res_gauss <- mclapply(1:rep.num, function(rep.sim){      # Use mclapply for parallel computation             
    ...
    if(rep.sim %% 200 == 0)  cat('---- Test times = ', rep.sim, '----\r',file = "ex2log.txt", append=TRUE)
  ...
  cat("---- result=\n", file = "ex2log.txt", append = TRUE)
  capture.output(print(result), file = "ex2log.txt", append = TRUE)
  cat("\n----\n", file = "ex2log.txt", append = TRUE)
  return(result)
}
...
for (n in nall) {
  ...
  for (dep in c(0, 1, 2)) {
    ...
    write.csv(
      result_m,
      paste0("ex2", "_n", n, "_p", p, "_a", alpha*100, "_dep", dep, ".csv"),
      row.names = FALSE
    )
