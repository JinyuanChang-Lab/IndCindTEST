...
Data1 <- function(n, p, q, dep){
  if(dep==0){
    x <- matrix(rt(n * p, df = 1), n, p)  # t(1)  
    y <- matrix(rt(n * q, df = 1), n, q)  # t(1)  
    dat <- list(x = x, y = y)
  } else {
    x <- matrix(rt(n * p, df = 1), n, p)  # t(1)  
    y1 <- exp(x[1:n, 1:dep])
    dat1 <- matrix(rt(n * (q - dep), df = 1), n, (q - dep))  # t(1)  
    y <- cbind(y1, dat1)
    dat <- list(x = x, y = y)
  }
  return(dat)  
...
res_my <- function(n, p, q, dep, alpha, rep.num, mycores){   
  ...
  res_gauss <- mclapply(1:rep.num, function(rep.sim){      # Use mclapply for parallel computation             
    ...
    if(rep.sim %% 200 == 0)  cat('---- Test times = ', rep.sim, '----\r',file = "ex1log.txt", append=TRUE)
  ...
  cat("---- result=\n", file = "ex1log.txt", append = TRUE)
  capture.output(print(result), file = "ex1log.txt", append = TRUE)
  cat("\n----\n", file = "ex1log.txt", append = TRUE)
  return(result)
}
...
for (n in nall) {
  ...
  for (dep in c(0, p/20, p/10)) {
    ...
    write.csv(
      result_m,
      paste0("ex1", "_n", n, "_p", p, "_a", alpha*100, "_dep", dep, ".csv"),
      row.names = FALSE
    )
