...
for (k in 1:d) {
  a1 <- paste0("2016-2018data/68", name[k], ".csv")
  x  <- read.csv(a1)
  ###get stock name
  a <- unique(x$PERMNO)
  num <- rep(0, length(a))
  ...
  a2 <- paste0("2016-2018data/68new_", name[k], ".csv")
  write.csv(A, a2)
  X <- X_o
  a3 <- paste0("2016-2018data/68X_", name[k], ".csv")
  write.csv(X, file = a3)
  fun_na <- function(x){
  ...
  #print(k)
  a4 <- paste0("2016-2018data/68all_", name[k], ".csv")
...
#######final data
for (k in 1: 11) {
  a5 <- paste0("2016-2018data/68all_", name[k], ".csv")
  ...
  a6 <- paste0("2016-2018data/68return_", name[k], ".csv")
...
for (k in 1:d) {
  a1 <- paste0("2020-2022data/02", name[k], ".csv")
  x  <- read.csv(a1)
  ###get stock name
  a <- unique(x$PERMNO)
  num <- rep(0, length(a))
  ...
  a2 <- paste0("2020-2022data/02new_", name[k], ".csv")
  write.csv(A, a2)
  X <- X_o
  a3 <- paste0("2020-2022data/02X_", name[k], ".csv")
  write.csv(X, file = a3)
  fun_na <- function(x){
    l <- which(is.na(x)==TRUE)
    if(length(l)>0) {return(length(l))
  ...
  #print(k)
  a4 <- paste0("2020-2022data/02all_", name[k], ".csv")
  write.csv(x_new_1, file = a4)
}
#####find unique NA-index
index_all  <- unique(de_idx)
...
#######final data
for (k in 1: 11) {
  a5 <- paste0("2020-2022data/02all_", name[k], ".csv")
  x_new_2 <- read.csv(a5)[, -1]
  ###delete all NA with same situation
  if(length(ind_my)>0){
    x_new_3 <- x_new_2[-ind_my, ]
  ...
  a6 <- paste0("2020-2022data/02return_", name[k], ".csv")
  write.csv(x_new_3, file = a6)
}
