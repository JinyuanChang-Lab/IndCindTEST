...
Data1 <- function(n, p, q, d){
  if(d==0){
    x <- matrix(rnorm(n*p, 0, 1), n)
    y <- matrix(rnorm(n*q, 0, 1), n)
  }else{
    Delta <- matrix(0, (p+q), (p+q))
    comp <- sample(c(1:(p+q)), 8, replace = FALSE, prob = rep(1/(p+q), (p+q)))
...
write.csv(result_m, paste0("ex4", "_n", n, "_p", p, "_a", alpha*100,"_dep",dep, ".csv"))
