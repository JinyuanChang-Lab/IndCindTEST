...
Data1 <- function(n, p, q, dep){
  if(dep==0){
    x <- matrix(rt(n * p, 1), n, p)
    y <- matrix(rt(n * q, 1), n, q)
    dat <- list(x=x, y=y)
  }else{
    if(dep==1){d <- p/20
...
write.csv(result_m, paste0("ex6", "_n", n, "_p", p, "_a", alpha*100,"_dep",dep, ".csv"))
...
#############dep=0##########
result_m <- res_my(n, p, q, dep, alpha, rep.num, mycores)
write.csv(result_m, paste0("ex1", "_n", n, "_p", p, "_a", alpha*100,"_dep",dep, ".csv"))
