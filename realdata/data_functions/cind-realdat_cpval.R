...
source('./data_functions_dep/cind-functions.R') 
source('./data_functions_dep/PCIND.R')

Rcpp::sourceCpp('./data_functions_dep/cv_hat_max_arma.cpp')


# ---- locate data/processed_data robustly (do not rely on working directory) ----
find_data_dir <- function(start = getwd(),
                          rel = file.path("data", "processed_data"),
                          max_up = 10) {
  p <- normalizePath(start, winslash = "/", mustWork = FALSE)
  for (k in 0:max_up) {
    cand <- file.path(p, rel)
    if (dir.exists(cand)) return(normalizePath(cand, winslash = "/", mustWork = TRUE))
    p2 <- dirname(p)
    if (identical(p2, p)) break
    p <- p2
  }
  stop("Cannot find 'data/processed_data' by searching upwards from: ", start)
}
...
res_gauss <- mclapply(1:rep.num, function(rep.sim){
  ...
  a1 <- file.path(DATA_DIR, paste0("68return_", com_my[1, i], ".csv"))
  dat1 <- read.csv(a1)
  a2 <- file.path(DATA_DIR, paste0("68return_", com_my[2, i], ".csv"))
  dat2 <- read.csv(a2)
  xx <- NULL;  yy <- NULL
  xx <- as.matrix(dat1)[,-1]
  yy <- as.matrix(dat2)[,-1]
  ...
  a3 <- file.path(DATA_DIR, paste0("68return_", name_z[1], ".csv"))    
  dat_z <- as.matrix(read.csv(a3))[, -1]
  if(length(name_z) > 1){
    for (j in 2:length(name_z)) {
      az <- file.path(DATA_DIR, paste0("68return_", name_z[j], ".csv"))    
      dat_e <- read.csv(az)
...
res_gauss <- mclapply(1:rep.num, function(rep.sim){
  ...
  res  <- rep(0, 8)
  coln <- paste0(com_my[1,i], "-", com_my[2,i])
  a1 <- file.path(DATA_DIR, paste0("02return_", com_my[1, i], ".csv"))
  dat1 <- read.csv(a1)
  a2 <- file.path(DATA_DIR, paste0("02return_", com_my[1, i], ".csv"))
  dat2 <- read.csv(a2)
  xx <- NULL;  yy <- NULL
  xx <- as.matrix(dat1)[,-1]
  yy <- as.matrix(dat2)[,-1]
  ...
  ind2 <- which(name==com_my[2,i])
  name_z <- name[-c(ind1, ind2)]
  a3 <- file.path(DATA_DIR, paste0("02return_", name_z[1], ".csv")) 
  dat_z <- as.matrix(read.csv(a3))[, -1]
  if(length(name_z) > 1){
    for (j in 2:length(name_z)) {
      az <- file.path(DATA_DIR, paste0("02return_", name_z[j], ".csv"))    
      dat_e <- read.csv(az)
      dat_z <- cbind(dat_z, as.matrix(dat_e)[,-1])
    }
  }
