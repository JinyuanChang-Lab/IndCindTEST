...
Args:
    setting: (1) "S-selectn3":  splitting sample with selecting n_3^{opt}  (see Algorithm 1, Section 4.1).
             (2) "S-givenn3":   splitting sample with given n_3 (see Remark 4, Section 4.1).
             (3) "full-sample": without sample-splitting (see Remark 4, Section 4.1).
    option_l: "Rademacher" / "Gaussian" / "Mammen" / "all"
...
def trainer(setting, exp_v, res_v, n, batchsize, lr, n_epochs, patience, input_features, hidden_features1, hidden_features2, output_features, device, thresh=None, drop_last1=True, plot=False):
    ...
    model = AdaptiveNet(input_features, hidden_features1, hidden_features2, output_features).to(device)
...
def gauss_fun_split(setting, option_l, x, y, z, n, alpha,batchsize, lr,n_epochs, patience, input_features, hidden_features1, hidden_features2, output_features,  drop_last1, device):
    ...
    if setting == "S-selectn3":
        ...
        if option_l in ("Rademacher", "all"): 
            ...
            test_1 = torch.max(abs(eps_t1.T @ delta_t1 / np.sqrt(n_tilde_red) ))   # calculate test statistic with selected n_3^{opt} (Rademacher)
        ...
        if option_l in ("Gaussian", "all"):
            ...
            test_2 =  torch.max(abs(eps_t2.T @ delta_t2 / np.sqrt(n_tilde_norm) ))  # calculate test statistic with selected n_3^{opt} (Gaussian)
        ...
        if option_l in ("Mammen", "all"):
            ...
            test_3 = torch.max(abs(eps_t3.T @ delta_t3 / np.sqrt(n_tilde_mm) )) # calculate test statistic with selected n_3^{opt} (Mammen's)
        ...
        if option_l in ("Rademacher", "all"):
            ...
            # reject the null hypothesis if rej_red1 = 1 
        ...
        if option_l in ("Gaussian", "all"):
            ...
            # reject the null hypothesis if rej_norm1 = 1
        ...
        if option_l in ("Mammen", "all"):    
            ...
            # reject the null hypothesis if rej_mm1 = 1
    ...
    if setting == "S-givenn3":
        ...
        ####Rademacher  
        if option_l in ("Rademacher", "all"): 
            n_tilde_red = n-n_s                         # n_3 = n - n_1 - n_2
            eps_t1 = eps_all[n_s:(n_s+n_tilde_red), :]
            delta_t1 = delta_all[n_s:(n_s+n_tilde_red), :]  
            test_1 = torch.max(abs(eps_t1.T @ delta_t1 / np.sqrt(n_tilde_red) ))   # calculate test statistic with given n_3  
            ### generate Radamacher multipliers (N-by-n_tilde_red matrix) 
            M = generate_one_minus_one_distribution(N, n_tilde_red).float().to(device)
            
        ####Gaussian
        if option_l in ("Gaussian", "all"):
            n_tilde_norm = n-n_s                         # n_3 = n - n_1 - n_2
            eps_t2 = eps_all[n_s:(n_s+n_tilde_norm), :]
            delta_t2 = delta_all[n_s:(n_s+n_tilde_norm), :]
            test_2 =  torch.max(abs(eps_t2.T @ delta_t2 / np.sqrt(n_tilde_norm) ))  # calculate test statistic with given n_3  
            ### generate Gaussian multipliers (N-by-n_tilde_norm matrix)
            M_norm = torch.randn(N, n_tilde_norm).to(device)
         
        ####Mammen's    
        if option_l in ("Mammen", "all"):
            n_tilde_mm = n-n_s                         # n_3 = n - n_1 - n_2
            eps_t3 = eps_all[n_s:(n_s+n_tilde_mm), :]
            delta_t3 = delta_all[n_s:(n_s+n_tilde_mm), :]
            test_3 = torch.max(abs(eps_t3.T @ delta_t3 / np.sqrt(n_tilde_mm) )) # calculate test statistic with given n_3  
            ### generate Mammen's multipliers (N-by-n_tilde_mm matrix)
            M_mm = generate_mammen(N,n_tilde_mm).float().to(device)
...
def gauss_fun_full(option_l,x, y, z, n, alpha,batchsize, lr,n_epochs, patience, input_features, hidden_features1, hidden_features2, output_features, drop_last1, device):    
    ...
    if option_l in ("Rademacher", "all"): 
        ...
        test_1 = torch.max(abs(eps_t1.T @ delta_t1 / np.sqrt(n_tilde_red) ))   # calculate test statistic  
        ### generate Radamacher multipliers (N-by-n_tilde_red matrix) 
        M = generate_one_minus_one_distribution(N, n_tilde_red).float().to(device)
    ...
    if option_l in ("Gaussian", "all"):
        ...
        delta_t2 = delta_all[n_s:(n_s+n_tilde_norm), :]
        #calculate test statistic
        test_2 =  torch.max(abs(eps_t2.T @ delta_t2 / np.sqrt(n_tilde_norm) ))  # calculate test statistic 
        ### generate Gaussian multipliers (N-by-n_tilde_norm matrix)
        M_norm = torch.randn(N, n_tilde_norm).to(device)
    ...
    if option_l in ("Mammen", "all"):
        ...
        delta_t3 = delta_all[n_s:(n_s+n_tilde_mm), :]
        #calculate test statistic
        test_3 = torch.max(abs(eps_t3.T @ delta_t3 / np.sqrt(n_tilde_mm) )) # calculate test statistic 
        ### generate Mammen's multipliers (N-by-n_tilde_mm matrix)
        M_mm = generate_mammen(N,n_tilde_mm).float().to(device)
    ...
    if option_l in ("Rademacher", "all"):
        ...
        # reject the null hypothesis if rej_red = 1
    ...
    if option_l in ("Gaussian", "all"):
        ...
        # reject the null hypothesis if rej_norm = 1
    ...
    if option_l in ("Mammen", "all"):    
        ...
        # reject the null hypothesis if rej_mm = 1
        rej_mm = (test_3 > cv_mm)
...
Args:
    setting: (1) "S-selectn3":  splitting sample with selecting n_3^{opt}  (see Algorithm 1, Section 4.1).
             (2) "S-givenn3":   splitting sample given n_3 (see Remark 4, Section 4.1).
             (3) "full-sample": without sample-splitting (see Remark 4, Section 4.1).
    option_l: "Rademacher" / "Gaussian" / "Mammen" / "all"
            "Rademacher" uses Rademacher multipliers;
            "Gaussian" uses Gaussian multipliers;
