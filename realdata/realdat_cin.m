...
for i=1:rep_num
    ...
    y=csvread(title_y,1,1);
    t = size(z_name, 2);
    title_z = strcat('68return_',z_name(1), '.csv');
    %z = importdata(title_z).data;
    z=csvread(title_z,1,1);
    for j=2:t
        title_zz = strcat('68return_',z_name(j), '.csv');
        %zz = importdata(title_zz).data;
        zz=csvread(title_zz,1,1);
        z =[z,zz];
    end
