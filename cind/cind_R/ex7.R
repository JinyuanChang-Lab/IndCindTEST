...
Data1<- function(n, p, m, dep) {
  ...
  z <- matrix(runif(n * m) * 2 - 1, nrow = n, ncol = m)  # 生成均匀分布的 z
  rho <- dep * 0.1
  ...
  for (i in 1:m) {
    ...
    eps_y <- rowSums(eps_y1)  # 按行求和
    y[, i] <- z[, i] + 0.25 * z[, i]^2 + eps_y
    
    eps_x <- rnorm(n,0,1)  # 生成正态分布的噪声
...
write.csv(result_m, paste0("cex7", "_n", n, "_p", p, "_a", alpha*100,"_dep",dep, ".csv"))
